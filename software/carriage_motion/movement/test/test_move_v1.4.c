  ...
  */
  ...
  #define SPEED_CONST 1 // multiplication constant for speed 
  ...
  void speed(int16_t actEnc[2], int16_t lstEnc[2], int actSpeed[2], int lstSpeed[2]){
    ...
    for(int i = 0; i<2; ++i){ 
      ...
      // number 50 is maximum speed of robot
      if(lstEnc[i] > 0 && lstEnc[i]+100 > 32767 && actEnc[i]+100 < lstEnc[i]){
         actSpeed[i] = (actEnc[i] - (lstEnc[i]-32767))*SPEED_CONST;
      }
      else if(lstEnc[i] < 0 && lstEnc[i]-100 < 32767 && actEnc[i]-100 > lstEnc[i]){
         actSpeed[i] = (actEnc[i] - (32767+lstEnc[i]))*SPEED_CONST;
      }

      else{
        actSpeed[i] = (actEnc[i] - lstEnc[i])*SPEED_CONST;
      }
    }
  }
  ...
  void regulMotor(int fd, unsigned char required[2], int actSpeed[2], int lstSpeed[2], int signal [2]){ 
    ...
    signal[i] += 1.9*(required[i]-actSpeed[i])-(actSpeed[i]-lstSpeed[i]); 
    maestroSetTarget(fd, i, signal[i]);
    }
  }
  // read and transform data from sensor to sensors array
  void sensor(int fd, unsigned char channel, unsigned char position, int array[4][MEASUREMENT_N], int distance[4]){
    int x;
...
if(sensdist[0] > 0 && sensdist[0] <= 400){
   reqSpeed[0] = 0;
   reqSpeed[1] = 0;
   checknum = 1;
 }
...
*/
 ...
 //printf("SPEED: %i, %i, %i, %i\n", actSpeed[0], actSpeed[1], sensdist[2], sensdist[3]);

 if(reqSpeed[0] == 0 && reqSpeed[1] == 0 && actSpeed[0] == 0 && actSpeed[1] == 0){
   if(stop == 60)break;
     ++stop;
   }
}
