//FIXME create licence
  ...
  */
  ...
  void sensor(int fd, unsigned char channel, unsigned char position, int array[4][MEASUREMENT_N], int distance[4], int lastdist[4]){
    ...
    if(x > 150 && x < 600) distance[channel-2] = (24141/(x-8))-0.17; //save the new distance value in sensor array - every element belong to one sensor -> 4 elemets in common
...
if(sensdist[0] > 0 && sensdist[0] <= 240 && checknum == 0){
  reqSpeed[0] = 0;
  reqSpeed[1] = 0;
  checknum = 1;
}
  ...
  else if(sensdist[2] >=190 || sensdist[1] == 0){
    reqSpeed[0] = FORWARD_SPEED;
...
printf("SPEED: %i, %i, %i, %i, %i, %i, %i, %i, %i\n", encodAct[0], encodAct[1], reqSpeed[0], reqSpeed[1], actSpeed[0], actSpeed[1], degree, diff, sensdist[0]);
}
