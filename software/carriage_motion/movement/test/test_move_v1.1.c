...
void sensor(int fd, int channel, int position, int array[4][MEASUREMENT_N], int distance[4]){
  ...
  if(x > 80 && x < 650) distance[channel-2] = (24141/(x-8))-0.17; //save the new distance value in sensor array - every element belong to one sensor -> 4 elemets in common
...
cfsetispeed(&options, B19200); //57600
...
/* 
 if(sensdist[0] > 0 && sensdist[0] <= 150){
   reqSpeed[0] = 0;
   reqSpeed[1] = 0;
 }
  if(sensdist[1] == 0){
...
*/
 printf("SPEED: %i, %i, %i\n", actSpeed[0], actSpeed[1], sensdist[0]);

 if(reqSpeed[0] == 0 && actSpeed[0] == 0 && actSpeed[1] == 0) break;
}
