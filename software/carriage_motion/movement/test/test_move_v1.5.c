//FIXME create licence
  ...
  */
  ...
  void sensor(int fd, unsigned char channel, unsigned char position, int array[4][MEASUREMENT_N], int distance[4], int lastdist[4]){
    ...
    if(x > 50 && x < 600) distance[channel-2] = (24141/(x-8))-0.17; //save the new distance value in sensor array - every element belong to one sensor -> 4 elemets in common
...
int difference(int16_t actEnc[2], int16_t lstEnc[2]){
  int16_t Value[2];
  for(int i=0; i<2; ++i){
    // 100 is constat for arbitrate if the comming value is overflow
    if(lstEnc[i]>0 && lstEnc[i]+100>32767 && lstEnc[i]>actEnc[i])
       Value[i] = actEnc[i]+32767;

    else if(lstEnc[i]<0 && lstEnc[i]-100<32767 && lstEnc[i]<actEnc[i])
      Value[i] = actEnc[i]-32767;

    else Value[i] = actEnc[i]; //the Value is not overflow
  }
...
void stopbarr(int fd, int actSpeed[2], int16_t reqSpeed[2], unsigned char *checknum, int time[2]){
  for(int i = 0; i<2; ++i){
    if(actSpeed[i] == 0 && reqSpeed[i] != 0){
      if(time[i] == 10){
       *checknum = 5; //the robot stuck on undetected barrieri
       //reqSpeed[0] = 0;
       //reqSpeed[1] = 0;
...
diff = difference(encodAct, encodLst); //the start encoder difference (mainly 0) 
//main cyclus with behaviour and controlling logic
for(; channel <= 6; ++channel){

if(sensdist[0] > 0 && sensdist[0] <= 300 && checknum == 0){
  reqSpeed[0] = 0;
  reqSpeed[1] = 0;
  checknum = 1;
}
  ...
  if(sensdist[1] >= 200 || sensdist[1] == 0){
...
/*
  else if(sensdist[2] >= 200 || sensdist[2] == 0){
    reqSpeed[0] = FORWARD_SPEED;
    reqSpeed[1] = REVERSE_SPEED;
  }

  else if(sensdist[3] >= 200 || sensdist[3] == 0){
    reqSpeed[0] = REVERSE_SPEED;
    reqSpeed[1] = REVERSE_SPEED;
  }
*/
...
//if(checknum == 0) //status 0 -> run straight 
  // straight(encodAct, encodLst, reqSpeed, NATIVE_SPEED, diff, overflow);
if(checknum != 5){
...
/*
if(starting > 50) 
  stopbarr(fd, actSpeed, reqSpeed, checknumpt, time);
else
  ++starting;
*/

//TODO if the robot doesn't move turn off the motors -> if nothing behind you reverse 10 cm the turn arround 90degrees
//make flexibility acceleration constant

 // 85 turn around 90 degrees
 if(checknum != 0){
   //checknum 1 -> control deceleration
   if(checknum ==1 && degree == 300){
     degree = 0;
     checknum = 2;
   }
   //checknum 3 -> rotate control
   else if(checknum == 3 && degree == 300){
     reqSpeed[0] = 0;
     reqSpeed[1] = 0;
     degree = 0;
     checknum = 4;
   ...
   else if(checknum == 4 && degree == 300){
     reqSpeed[0] = FORWARD_SPEED;
     reqSpeed[1] = FORWARD_SPEED;
     diff = difference(encodAct, encodLst);
     //starting = 0;
     checknum = 0;
     degree = 0;
   }
...
printf("SPEED: %i, %i, %i, %i, %i, %i, %i\n", encodAct[0], encodAct[1], reqSpeed[0], reqSpeed[1], diff, overflow[0], overflow[1]);
}
