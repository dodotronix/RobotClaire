//FIXME create licence
  ...
  */
  ...
  #define SLIPP_CONST 202 // constant to control encoders motion
  #define WAIT_CONST 150
...
void stopbarr(int fd, int actSpeed[2], int16_t reqSpeed[2], unsigned char *checknum, int time[2], int PWM[2]){
  for(int i = 0; i<2; ++i){
    if(actSpeed[i] == 0 && PWM[i] > 6600 && reqSpeed[i] != 0){
      if(time[i] == 10){
        *checknum = 10; //the robot stuck on undetected barrieri
        time[0] = 0;
        time[1] = 0;
      }
      else time[i]+=1;
...
*/
 ...
 if(channel == 6){
   channel = 2;
   ++position; 
   if(position == MEASUREMENT_N) position = 0; //all position in sensor array full?
 }
...
if(checknum == 0) //status 0 -> run straight 
 ...
 if(checknum != 0){
   ...
   //reverse
     ...
     if(sensdist[3] <= 200 && sensdist[3] > 0){ 
       reqSpeed[0] = 0;
       reqSpeed[1] = 0;
       checknum = 11;
     }  
   ...
   else if(checknum == 11 && sensdist[3] == 0){                                  
     reqSpeed[0] = REVERSE_SPEED;
     reqSpeed[1] = REVERSE_SPEED;
     checknum = 5;
   }
...
//printf("SPEED: %i, %i, %i, %i, %i, %i, %i, %i, %i, %i\n", encodAct[0], encodAct[1], reqSpeed[0], reqSpeed[1], actSpeed[0], actSpeed[1], PWM[0], PWM[1], diff, sensdist[0]);
}
