...
#define TRIGHT 0 //turn right macro
#define TLEFT 1 //turn left macro
...
void turn(uint8_t dir, uint16_t tics){
  ...
  while(1){
    switch (*modept){
      ...
      case 1:
        ...
        if(allsens ==5){
          ...
          while(*modept ==1){
            ...
            if(node ==0){
              sens[0] = vlx_sing_shot(FRONT0);
              sens[2] = vlx_sing_shot(FRONT1) 
              if(sens[0] <255 || sens[2] <255){
                req_spd[0] =0;
                req_spd[1] =0;
                node =1; //set control node number
              }
            while(!(node ==1 && curr_spd[0] ==0 && curr_spd[1] ==0))//wait till the robot stay
              node =2;
            }
            //where can the robot go?
              ...
              if(vlx_sing_shot(RIGHT) ==255){
                ...
                req_spd[0] =-8;
                req_spd[1] =8;
                while(!(motion == ROTATE)); //rotate 90degree
                motion =-1;
                node=3;
              }
      ...
      case 2:
        ...
        /* 
          servo_pos(servo2, 1000);
          servo_pos(servo3, 1100);
          servo_pos(servo1, 800);
          servo_pos(servo3, 1100);
          servo_pos(servo1, 400);
          servo_pos(servo2, 840);
          *servo5 =900;
          _delay_ms(2000);
        */
          //servo_pos(servo2, 1000);
