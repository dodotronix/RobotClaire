...
volatile uint8_t req_spd [] = {0, 0}; //required speed, this variable change speed of robot
...
void servos_init(){
  ...
  */
  ...
  TCF1.INTCTRLA = TC_OVFINTLVL_LO_gc; //set interrupt(A) generation 
  TCF1.PER = PWM_PER; //period
  TCF1.CTRLA = Prescaler; //prescaler
}

ISR (TCF1_OVF_vect){
  front[0] = vlx_sing_shot(FRONT0); //measure distance with freq: 50Hz
  //front[1] = vlx_sing_shot(FRONT1);
}
  ...
  while(1){
    switch (*modept){
      ...
      case 1:
        ...
        req_spd[1] = 5;
        while(*modept == 1){
          if(front[0] < 255) PORTA.OUTSET = (1<<PIN4); 
          else PORTA.OUTCLR = (1<<PIN4);
          //if(front0 <255) PORTA.OUTSET = (1<<PIN6);
