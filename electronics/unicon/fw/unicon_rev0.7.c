...
#define ROTATE 680 //number of pulse to turn 90degree
#define SPEED 5 //speed of arm moves
#define TRIGHT 1 //turn right macro
#define TLEFT 2 //turn left macro
#define STRAIGHT 0
#define BACKWARD 3
...
volatile int motion =-1; //save actual value of encoder
...
//volatile uint8_t *pReg = (voilatile uint8_t *) &PORTE_IN;
...
ISR (TCD1_OVF_vect){
  ...
  TCD1.CCA += (req_spd[0]-curr_spd[0])-(curr_spd[0]-last_spd[0]);//PD regulator
}
...
ISR (TCE0_OVF_vect){
  ...
  TCE0.CCD += (req_spd[1]-curr_spd[1])-(curr_spd[1]-last_spd[1]);// PD regulator
}
...
ISR(PORTD_INT0_vect){
    ...
    if(motion >=0) ++motion; //if robot rotates -> count pulses
}
...
void exact_count(uint8_t dir, uint16_t tics){
  motion =0; //activate pulse counting
  if(dir ==0){
    //go straight
    req_spd[0] =4;
    req_spd[1] =4;
  ...
  while(!(motion == tics)); //rotate 90degree
  motion =-1; //deactive pulse counting
  req_spd[0] =0;
  req_spd[1] =0;
}
...
int main(){
  ...
  uint8_t dist[] = {0, 0, 0, 0}; //check if in front of the robot is a barrier
      ...
      case 1:
        ...
        if(allsens ==5){
          ...
          while(*modept ==1){
            ...
            if(node ==0){
              dist[0] = vlx_sing_shot(FRONT0);
              dist[2] = vlx_sing_shot(FRONT1); 
              if(dist[0] <255 || dist[2] <255){
                req_spd[0] =0;
                req_spd[1] =0;
                while(!(curr_spd[0] ==0 && curr_spd[1] ==0));//wait till the robot stay
                exact_count(TRIGHT, 100); //look right
                dist[1] = vlx_sing_shot(FRONT0); //measure distance
                if(dist[1] <255){
                  exact_count(TLEFT, 100);
                  node =1; //set control node number
                }
                else{
                  exact_count(TLEFT, 100); //turn back
                  exact_count(STRAIGHT, 100); //go nearer to the object
                  dist[1] = vlx_sing_shot(FRONT0);
                  dist[3] = vlx_sing_shot(FRONT1);
                  while(take_object(dist) !=0);
                  exact_count(TLEFT, ROTATE); //turn back
                  exact_count(TLEFT, ROTATE); //turn back
                  req_spd[0] =4;
                  req_spd[1] =4;
                  node =0;
            ...
            //where can the robot go?
              ...
              if(vlx_sing_shot(RIGHT) ==255){
                exact_count(TRIGHT, ROTATE); //turn right
              } 
              else if(vlx_sing_shot(LEFT) ==255){
                exact_count(TLEFT, ROTATE); //turn left
              }
              ...
              //may i go lef or right direction?
              if(vlx_sing_shot(RIGHT) ==255 || vlx_sing_shot(LEFT) ==255){
                if(motion ==-1) motion =0; //activate pulses counting
                if(vlx_sing_shot(BACK) ==255 && motion ==600){
                  req_spd[0] =0;
                  req_spd[1] =0;
                  motion =-1; //deactive pulses counting
                  node =1; //return to note 2
                }
                else if(vlx_sing_shot(BACK) <255 && motion !=600){
                  req_spd[0] =0;
                  req_spd[1] =0;
                  motion =-1; //deactive pulse counting
                  node =1;
