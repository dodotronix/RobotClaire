...
#define SPDMOV 10
#define ERROR 5
#define MEASUREMENT_N 20 //number of measure samples
...
uint8_t all[] ={0x06, 0x01}; //turn on all LEDs
...
void insertionSort(uint8_t num, uint8_t array[], uint8_t loc) {
  uint8_t k = loc;
  if (loc > 0) {
    while (array[k-1] > num && k != 0) {
      array[k] = array[k-1];
      --k;
    }
    array[k] = num;
...
uint8_t filter_median(uint16_t addr) {
  uint8_t array[MEASUREMENT_N];
  uint8_t num =0;
  for (uint8_t i =0; i < MEASUREMENT_N; ++i){
    num = vlx_sing_shot(addr);
    insertionSort(num, array, i);
  }

  return (array[MEASUREMENT_N/2 -1] + array[MEASUREMENT_N/2]) / 2;
}


void sens_calib(int16_t turn){
  //uint16_t pulse =620 +(3*turn)/8;
  uint16_t pulse =615 +turn; //618
      ...
      *movpt =1;
      ...
      if(sens_num ==5){
        ...
        *movpt =1; //start moving
        while(*modept ==2){
          if(node ==0){
            if(vlx_sing_shot(FRONT0) <100 || vlx_sing_shot(FRONT1) <100){
              PORTA.OUTSET =(1<<PIN4);
              req_spd[0] =0;
              req_spd[1] =0;
              ...
              sensors[1] =filter_median(FRONT1);
              sensors[0] =filter_median(FRONT0);

            if(sensors[0] <110){
              num =(sensors[0] - sensors[1])/5;   
            }
            else if(sensors[1] <110){
              num =(sensors[0] - sensors[1])/5; 
            }
            else
              num =(3*(sensors[0] - sensors[1]))/5;
            take_object(num);
            enk_clear(); //set encoders to zero
            _delay_ms(500);
            *movpt =2;
            ...
            *movpt =1;
            ...
            if(vlx_sing_shot(FRONT0) <130 || vlx_sing_shot(FRONT1) <130){
              PORTA.OUTSET =(1<<PIN4);
              req_spd[0] =0;
              req_spd[1] =0;
              while(!(curr_spd[0] ==0 && curr_spd[1] ==0));//wait till the robot stay
      ...
      if(twi_write(LED_DVR, all, 2) !=2) PORTA.OUTSET |= (1<<PIN5);
      PORTA.OUTCLR = (1<<PIN5) | (1<<PIN6) | (1<<PIN4); //clear error indicator 
      _delay_ms(500);
      servo_pos(servo2, 810);
      servo_pos(servo3, 600);
      *servo5 =900; 
      servo_pos(servo1, 670);
      servo_pos(servo0, 612);


      while(*modept ==3){
        if(vlx_sing_shot(FRONT0) <105 || vlx_sing_shot(FRONT1) <105){
          sensors[1] =filter_median(FRONT1);
          sensors[0] =filter_median(FRONT0);
          if(sensors[0] <110 && sensors[1] <110){
            PORTA.OUTSET =(1<<PIN4) | (1<<PIN6);
            num =(3*(sensors[0] - sensors[1]))/5;
            //sens_calib(num);
          }
          else if(sensors[0] <110){
            //LEFT side
            num =(sensors[0] - sensors[1])/5; 
            PORTA.OUTSET =(1<<PIN4);
            PORTA.OUTCLR =(1<<PIN6);
