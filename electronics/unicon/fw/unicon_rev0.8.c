...
uint16_t set_position(int16_t req_pos, uint8_t id){
  ...
  if(id ==0){
    pTerm =(req_pos -enk_val[0]); //div4
    dTerm =10*(enk_val[0] -dState[0]); //10*
    dState[0] =enk_val[0];
    result = pTerm -dTerm;
  }

  else if(id ==1){
    pTerm =(req_pos -enk_val[1]); // div4
    dTerm =10*(enk_val[1] -dState[1]);
    dState[1] =enk_val[1];
    result = pTerm -dTerm;
  }
...
ISR (TCE0_OVF_vect){
  last_spd[1] = curr_spd[1]; //save curr_speed value as last_speed value
