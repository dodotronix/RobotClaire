...
//promenna Enkoderu0
volatile int EnkValue0 = 0; // 32-bit promenna pro zaznamenavani hodnot z enkoderu

//promenna Enkoderu1
volatile int EnkValue1 = 0; // 32-bit promenna pro zaznamenavani hodnot z enkoderu
...
unsigned char SplitNumber (int y, unsigned char poradi)
{
...
ISR (INT0_vect) // vector INT0 (PD2)
{
      ...
      {
        EnkValue0 -= 1;
        if(EnkValue0 == -32767) EnkValue0 = 0;
      }
...
ISR (INT1_vect) //vektor INT1 (PD3)
{
      ...
      {
        EnkValue1 -= 1;
        if(EnkValue1 == -32767) EnkValue1 = 0;
      }
...
{// knihovna WiringPi neumi posilat bajty zasebou s repeated startem ale jen oddelene, proto jsou jednotlive cteni oddeleny NACK bitem
  ...
  { 
        ...
        if((TWSR & 0xF8)==0xA8) // ATmega prijala SLA+R od MASTERa a vratila ACK
        {
            ...
            TWDR = SplitNumber (Value,(2-pocet));
            --pocet;
	  ...
	  pocet = 2; // pocet bajtu, ze kterych je cislo slozeno?
